...
function LoginPage({ onLogin, registeredUsers, onRegister }) {
  ...
  function handleRegister() {
    ...
    onRegister(newUser);
    // Auto-login immediately after registration — no need to login again
    onLogin({ type: "user", data: newUser });
  }
    ...
    async function loadTodayLog() {
      try {
        ...
        if (fbUser && fbUser.log) {
          ...
          if (todayLog && todayLog.foods) setTodayFoods(todayLog.foods);
        }
  ...
  useEffect(() => {
    ...
    async function saveLog() {
      ...
      try {
        ...
        const newEntry = {
          date: today, foods: todayFoods,
          totalCal: Math.round(m.cal), totalProt: Math.round(m.prot),
          totalCarbs: Math.round(m.carbs), totalFat: Math.round(m.fat),
        };
        const snapshot = await getDocs(collection(db, "users"));
        const fbUser = snapshot.docs.map(d => d.data()).find(u => u.id === user.id);
        const existingLog = (fbUser && fbUser.log) ? fbUser.log : (user.log || []);
        const updatedLog = [...existingLog.filter(l => l.date !== today), newEntry];
...
export default function FitFuelApp() {
  // Restore session from localStorage so user stays logged in after closing app
  ...
  const [needsOnboard, setNeedsOnboard] = useState(() => {
    try { return localStorage.getItem("samfit_onboard") === "true"; } catch { return false; }
  });
  const [registeredUsers, setRegisteredUsers] = useState([]);
  const [loading, setLoading] = useState(true);

  // Load all registered users from Firebase on startup
  useEffect(() => {
    async function loadUsers() {
      try {
        const snapshot = await getDocs(collection(db, "users"));
        const fbUsers = snapshot.docs.map(d => d.data());
        setRegisteredUsers(fbUsers);
        // If user is logged in, refresh their data from Firebase too
        const savedAuth = localStorage.getItem("samfit_auth");
        if (savedAuth) {
          const parsedAuth = JSON.parse(savedAuth);
          if (parsedAuth.type === "user") {
            const freshUser = fbUsers.find(u => u.id === parsedAuth.data.id);
            if (freshUser) {
              setAuth({ type: "user", data: freshUser });
              if (freshUser.caloricTarget) {
                setUserData(freshUser);
                localStorage.setItem("samfit_user", JSON.stringify(freshUser));
              }
            }
          }
        }
  ...
  function handleLogin(res) {
    ...
    if (!u.caloricTarget) {
      ...
      try { localStorage.setItem("samfit_onboard", "true"); } catch {}
    } else {
      setUserData(u);
      try { localStorage.setItem("samfit_user", JSON.stringify(u)); } catch {}
    }
  }
  ...
  async function handleOnboard(form) {
    ...
    try {
      localStorage.setItem("samfit_auth", JSON.stringify({ type: "user", data: u }));
      localStorage.setItem("samfit_user", JSON.stringify(u));
      localStorage.removeItem("samfit_onboard");
    } catch {}
  }
  ...
  function handleLogout() {
    ...
    try {
      localStorage.removeItem("samfit_auth");
      localStorage.removeItem("samfit_user");
      localStorage.removeItem("samfit_onboard");
    } catch {}
  }
